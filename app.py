...
# Directories for data, downloading, and uploading files
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
UPLOAD_FOLDER = os.path.join(BASE_DIR, 'files', 'resumes')
DOWNLOAD_FOLDER = os.path.join(BASE_DIR, 'files', 'outputs')
...
def process():
    ...
    try:
        ...
        skills_file = os.path.join(DATA_FOLDER, 'skill_red.csv')
        if not os.path.exists(skills_file):
            raise FileNotFoundError(f"Skills file not found: {skills_file}")

        skills = pd.read_csv(skills_file)
        skill_list = [s.lower() for s in skills.values.flatten()]
